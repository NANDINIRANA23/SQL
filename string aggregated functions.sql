...
select e1.emp_name,e2.emp_name as manager_name,datediff(year,e1.dob,e2.dob) as age_diff
from employee e1
...
select e2.emp_name as manager_name,STRING_AGG(e1.emp_name,',')as employees,e2.salary
from employee e1
inner join employee e2 on e1.manager_id=e2.emp_id
group by e2.emp_name, e2.salary
order by e2.salary
...
select order_date,ship_date,datediff(dd,order_date,ship_date)-2*datediff(ww,order_date,ship_date) as business_days
from orders
...
select o.category, sum(sales) as total_sales,sum (case when r.order_id is not null then sales end) as return_sales
from orders o 
left join returns r on o.order_id=r.order_id
group by o.category
...
select category,
sum(case when datepart(year,order_date) = 2019 then sales end) as total_sale_2019,
sum(case when datepart(year,order_date) = 2020 then sales end) as total_sale_2020
from orders 
group by category
...
select top 5 city ,avg(datediff(dd,order_date,ship_date)) as avg_days
from orders
where region='West'
group by city
...
select string_agg(e1.emp_name,',')as emp,string_agg(e2.emp_name,',') as manager_name,e3.emp_name as senior_manager
from employee e1
inner join employee e2 on e1.manager_id=e2.emp_id
inner join employee e3 on e2.manager_id=e3.emp_id
group by e1.emp_name,e2.emp_name,e3.emp_name
